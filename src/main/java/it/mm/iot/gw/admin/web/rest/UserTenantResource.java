...
public class UserTenantResource extends AbstractRestService {
    ...
    public ResponseEntity<InfoUserMessage<InfoUserOutput>> getUserInfoComplete(@RequestBody InfoUserRequest infoUserRequest) {
        InfoUserOutput out = userService.getInfoUser(infoUserRequest);

        return new ResponseEntity<>(OutputMessageFactory.create(InfoUserMessage.class, out, getOperationOutcomeAndRemove()), HttpStatus.OK);
    }
}
